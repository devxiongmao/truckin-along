...
RSpec.describe ShipmentDeliveryMailer, type: :mailer do
  describe '#partially_delivered_email' do
    let(:user) { create(:user, role: "customer") }
    let(:shipment) { create(:shipment, user: user) }

    let(:mail) { described_class.partially_delivered_email(shipment.id) }

    it 'renders the headers' do
      expect(mail.subject).to eq('Your package has been delivered a partial distance!')
